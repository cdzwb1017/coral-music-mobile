...
- `UI-16 [DOING]` 推荐、搜索、歌单和播放队列共用下载反馈/状态按钮；歌曲收藏显示真实实心状态和反馈；我的列表、我的收藏、下载、设置补可见返回及系统返回栈。记录：`development-history/2026-07-20-136-ui-16-list-actions-and-back-navigation.md`。
- `UI-17 [DOING]` 单曲下载按钮点击首帧显示加载动画，队列下载中持续显示状态，完成后显示勾；播放详情与所有歌曲列表复用同一按钮。记录：`development-history/2026-07-20-140-ui-17-download-button-state.md`。
- `UI-18 [DOING]` 首页铃铛显示消息空态，音乐平台选择改为主题化锚点菜单并保留真实来源切换。记录：`development-history/2026-07-20-142-ui-18-home-topbar-menus.md`。
- `UI-19 [DOING]` 搜索页音乐平台菜单与首页统一为主题化锚点菜单，记录：`development-history/2026-07-21-147-ui-19-search-source-menu.md`。
- `UI-20 [DOING]` 在线封面加载统一与播放队列视觉优化，记录：`development-history/2026-07-21-148-ui-20-cover-loading-and-queue.md`。
- `B3-23/B4-37 [DOING]` 多来源搜索、封面、歌词与播放器焦点修复，记录：`development-history/2026-07-21-149-multisource-regressions.md`。
...
- `P1-03 [DOING]` 已建立 `go_router` 九入口、手机底栏、宽屏 Rail、迷你播放栏和独立播放详情页；壳外播放器进入壳内功能统一使用位置切换，避免重复构建 `StatefulShellRoute`；“我的”子页面使用压栈和共享返回兜底，页面按钮与系统返回均可回到上层。播放器批次仍需补质量、队列与收藏。
...
- `P2-04 [DOING]` B3-05/B3-10 已实现酷我热门歌单、HTTPS 分类标签、排序、搜索、详情和播放入口；B3-17/B3-18 已接入 QQ 与咪咕的热门/最新或推荐歌单、标签（咪咕）和详情；B3-20 已移除页码控件，统一为触底追加、跨页去重和末页停止；B3-22 按桌面端规则修复酷我非 digest-8 歌单需先解析真实 PID 的详情路由；B3-24 补齐 QQ HTTPS 与咪咕签名的关键词搜索；B4-46 至 B4-49 已接入网易云推荐、酷狗 HTTPS 广场/详情/搜索，补齐酷狗榜单批量封面、网易云 EAPI 搜索与广场封面、酷狗新详情脚本、咪咕专辑图和 QQ 当前官方详情接口，并让三个在线页面只显示当前音源声明且该页面已实现的平台。B5-13 已实现本地收藏快照，B6-02 已实现歌单下载全部。记录：`development-history/2026-07-20-087-b5-13-online-playlist-favorite-snapshots.md`、`development-history/2026-07-20-125-b3-17-qq-playlist-plaza.md`、`development-history/2026-07-20-126-b3-18-migu-playlist-plaza.md`、`development-history/2026-07-20-137-b3-20-playlist-infinite-scroll.md`、`development-history/2026-07-20-145-b3-22-kuwo-playlist-detail-routing.md`、`development-history/2026-07-21-151-b3-24-qq-migu-playlist-search.md`、`development-history/2026-07-23-b4-46-source-capability-menus-and-kugou-plaza.md`、`development-history/2026-07-23-b4-47-search-playlist-and-netease-artwork.md`、`development-history/2026-07-23-b4-48-home-artwork-and-plaza-sources.md`、`development-history/2026-07-23-b4-49-playlist-detail-compatibility.md`。
- `P2-05 [DOING]` 已实现酷我分页榜单及 QQ 当前榜单目录/详情，支持来源切换和跨导航状态；B3-21 已增加按本地日期稳定轮换真实榜单的每日推荐，并将原无效排行榜快捷入口替换为过滤不感兴趣歌曲、立即随机播放的音乐电台。其余来源待迁移，跨来源缓存待后续。记录：`development-history/2026-07-20-141-b3-21-daily-recommendation-radio.md`。
...
- `P3-02 [DOING]` B4-01 已实现在线 `PlaybackResolver` 与 Android 受限 User API `musicUrl`，并通过真机 `kw` 取链；B4-12 已完成会话内音源管理，支持从原 HTTPS 地址刷新并在失败时恢复旧脚本；B4-13 已实现 URL 缓存/刷新，且脚本导入、启用、刷新或清除成功后会清空会话 URL 缓存，避免旧脚本地址复用；播放详情的用户主动重试也会强制重新取链。B4-36 已让启动脚本恢复、上次在线歌曲恢复和在线取链共享同一就绪屏障，发现页切换目录平台不再竞争或重置 User API 运行时。B4-40 补齐 QQ `strMediaMid/types` 与咪咕 `copyrightId` 等桌面 User API 字段，保证可搜索曲目能以相同协议取链。User API 返回的实际质量 type 会随缓存传入播放器，避免降档流仍显示 SQ；B4-19 已实现已声明质量内的降级重试。B4-26 已在真实 SQ/FLAC 流以 Range 总大小和 FLAC 总采样数显示平均 `1643 kbps · 44 kHz · FLAC · SQ`，未探测到的文件参数不再以理论规格伪回退。B4-27 已完成 iOS WKWebView/同步 MD5 的同协议编译校验，真机/真实脚本验证仍待平台 Runtime；本地、下载和 WebDAV 已直连各自地址且不走 User API；来源发现、鉴权 Range 和跨来源换源仍待后续扩展。
- `P3-03 [DOING]` B4-03 已实现队列首尾循环的上一首/下一首与详情页控制；详情页下一首已在 Android 真机切至《红尘客栈》并立即恢复 `PLAYING`。B4-04/B4-05 已在 Android 真机验证列表循环、单曲循环与随机自动切歌；B4-33 已修复重复完成快照连续推进队列、替换队列重置模式，以及随机模式手动切歌仍按相邻项的问题，自动测试与三模式真机回归待补。B4-14/B4-15 已在真实 30 首队列验证非当前删除和拖动排序均不打断当前播放；B4-31 已将队列曲目、索引、模式和上下文安全恢复到 SQLite，但不自动播放。B4-06 失效音源自动跳过及其余平台回归待补。记录：`development-history/2026-07-20-114-b4-31-playback-queue-persistence.md`、`development-history/2026-07-20-138-b4-33-playback-mode-advance-fix.md`。
...
- `P3-06 [DOING]` B4-02 已实现歌词阅读入口，B4-17 已接通原文/翻译/罗马音与 LX 逐字 LRC、当前行高亮及 LRC `offset`；B4-35 将歌词服务从 User API 取链中分离；B4-39 让本地歌曲在同目录 LRC 缺失时按歌名、歌手使用独立搜索，下载/WebDAV 保持离线；B4-40 将所有在线与本地曲目统一为独立歌词聚合器，彻底删除原生及 User API 歌词调用；B4-41 将网易云搜索切至落雪同款 EAPI，酷狗歌词升级为 HTTPS、按曲目选择候选并解析 KRC 为 LX 逐字时间，网易云读取可用 YRC 为 LX 逐字时间，搜索入口不再依赖播放脚本声明；B4-42 改为 LrcLib 按歌名、歌手（可用时加专辑、时长）独立检索优先，公开平台端点仅未命中时兜底，并让音乐分类、歌单广场、网盘资源系统返回回到“更多”；B4-44 将网易云封面在曲目模型层改为 HTTPS，并接通歌单搜索与详情。Android 真机逐字填充仅作用于当前行、完成行恢复普通色且无绿色进度条；当前启用音源变更会使同一曲目歌词自动重新获取，空态/失败态可手动重新加载，iOS/鸿蒙来源优先级与真机兼容待补。记录：`development-history/2026-07-20-144-b4-35-independent-lyric-services.md`、`development-history/2026-07-21-153-b4-39-local-lyric-fallback-and-format-matrix.md`、`development-history/2026-07-21-156-b4-40-qq-migu-playback-and-lyrics.md`、`development-history/2026-07-23-b4-41-online-source-lyrics-and-back.md`、`development-history/2026-07-23-b4-42-independent-lyric-search-and-more-back.md`、`development-history/2026-07-23-b4-44-netease-search-artwork-and-playlists.md`。
...
- `P4-04 [DOING]` B5-11 已接入系统文件选择与目录递归扫描、格式过滤、CUE、同目录 LRC 与 MP3/AAC ID3、FLAC、WAV、M4A、Ogg/Opus 的共享标签/封面读取，并写入现有列表、队列与 `AudioEngine`；B5-20 补齐 Android 目录媒体读取授权、拒绝/遍历失败反馈及 DSD/AC3/AIFF/M4R/WMA 后缀识别；B5-21 允许无播放列表时直接选择音频并自动建立“本地音乐”；B4-39 覆盖 FLAC 扫描、标签与本地 URI 直通；B5-22 已用非 GPL FFmpegKit 将 Android DSF/DFF/WAV 异步转为临时 PCM WAV 以恢复兼容播放，桌面同等秒播需后续原生 Media3 数据源实现。Android 真机目录回归、目录授权恢复和三端真实格式解码验收待完成。记录：`development-history/2026-07-20-090-b5-11-local-audio-import.md`、`development-history/2026-07-21-152-b5-20-android-local-directory-access.md`、`development-history/2026-07-21-153-b4-39-local-lyric-fallback-and-format-matrix.md`、`development-history/2026-07-21-155-b5-21-empty-library-local-import.md`、`development-history/2026-07-21-157-b5-22-android-dsd-streaming-decoder.md`。
...
- `P6-01 [DOING]` B4-01 已提前受限 `musicUrl` 小样，B4-12 已实现脚本 URL、文件与粘贴导入、启用/删除与能力展示；B4-30 已将 HTTPS 来源地址/名称安全持久化，B4-34 已在应用根启动阶段自动重新校验加载，不再依赖进入音源管理；B4-38 在新安装且无用户来源时，使用同一受限 HTTPS 导入链路配置公开落雪音源，已有用户来源始终优先，本地会话脚本不保存。完整隔离和商店版门控仍在本阶段。记录：`development-history/2026-07-20-105-b4-30-user-api-url-persistence.md`、`development-history/2026-07-20-139-b4-34-startup-user-api-restore.md`、`development-history/2026-07-21-150-b4-38-default-lx-source.md`。
